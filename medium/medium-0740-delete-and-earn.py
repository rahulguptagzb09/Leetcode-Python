...
class Solution:
    def deleteAndEarn(self, nums: List[int]) -> int:
        ...
        for i in range(len(nums)):
            ...
            # cant use both cur_earn and earn_2
            if i > 0 and nums[i] == nums[i - 1] + 1:
                temp = earn_2
                earn_2 = max(cur_earn + earn_1, earn_2)
                earn_1 = temp
            else:
                temp = earn_2
                earn_2 = cur_earn + earn_2
                earn_1 = temp
        return earn_2

sol = Solution()
print(sol.deleteAndEarn(nums = [3,4,2]))
print(sol.deleteAndEarn(nums = [2,2,3,3,3,4]))
